...
public class EnemyBuff : MonoBehaviour 
{
    ...
    public void AssignAttackEvents(Enemy enemy) {
        enemy.enemyBuffIDs.ForEach(ID => {
            UnityEvent<object, GameObject> nowAttackEvent = new UnityEvent<object, GameObject>();
            switch (ID) {
                case 0:
                    nowAttackEvent.AddListener(Poison);
                    enemy.AddAttackEvent(nowAttackEvent);
                    break;
                case 1:
                    nowAttackEvent.AddListener(Slowness);
                    enemy.AddAttackEvent(nowAttackEvent);
                    break;
                default:
